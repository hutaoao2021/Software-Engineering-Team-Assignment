...
import com.yrp.pojo.ShoppingCart;
import com.yrp.service.ShoppingCartService;
...
public class ShoppingCartController {
    ...
    // 	http://localhost/shoppingCart/sub
    @PostMapping("/sub")
    public R<ShoppingCart> sub(@RequestBody ShoppingCart shoppingCart){
        //1、 获取当前用户的id
        Long currentId = BaseContext.getCurrentId();
        //2、根据当前用户的id 与 菜品的id 或 套餐的id查询当前用购物车中是否存在该套餐或者菜品
        // 菜品id
        ...
        if (one != null){
            ...
            // 当购物车中该物品或者套餐的数量为0的时候，将其从购物车中删除
            if (one.getNumber() == 0){
                shoppingCartService.removeById(one);
            }
            shoppingCartService.updateById(one);
        }else{
            throw new BusinessException(Code.BUSINESS_ERR,"当前菜品/套餐已经为0");
        }
